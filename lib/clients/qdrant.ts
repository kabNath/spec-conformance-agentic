...
// Lazily construct the client on first use (not at import) so `next build` and
// other tooling can load this module without QDRANT_URL set.
let _qdrant: QdrantClient | null = null;
export function qdrant(): QdrantClient {
  return (_qdrant ??= new QdrantClient({
